...
- **koffi 固定为 3.1.1**——koffi 3.1.3 起的 win32-x64 预编译二进制会在 `koffi.load()` 内以访问违例崩溃（[上游回归](https://github.com/Koromix/koffi/issues/275)）；桌面运行时在 deploy 时会重新解析 `^` 范围，因此只有精确固定才能让打包后的选择器停留在可用版本。上游发布修复后的 win32-x64 预编译二进制时再重新评估该固定。
